<?php
	...
	$query = "SELECT `id` FROM `t2b_users` WHERE `num`='$id'";
	$result = mysqli_query($dbc, $query) or die ('Такого пользователя не существует!');
	$row = mysqli_fetch_assoc($result);
	$id = $row['id'];
	if(empty($id)){
		header('Location: http://www.t2b-pro.ru');
	}
	//Проверяем заполнена ли информация о пользователе
	$query = "SELECT * FROM `t2b_user_info` WHERE `id`='$id'";
	$result = mysqli_query($dbc, $query) or diw ('Не удалось извлесь информацию о пользователе');
	$row_user = mysqli_fetch_assoc($result);
	$company = $row_user['company'];
	$inn = $row_user['inn'];
	$logo = $row_user['logo'];
	$tel = $row_user['tel'];
	$region = $row_user['region'];
	$name = $row_user['name'];
	$familie = $row_user['familie'];
	$father = $row_user['father'];
            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-md-8 col-sm-6 col-xs-6">
                        ...
                        <div class="main-menu">
                            <ul>
                                <li><a href="#front">Кабинет</a></li>
                                <li><a href="#services">Купить</a></li>
                                <li><a href="#products">Продать</a></li>
                                <li><a href="#contact">Результаты</a></li>
                            </ul>
                        </div> <!-- /.main-menu -->
                    </div> <!-- /.col-md-8 -->
                </div> <!-- /.row -->
        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-md-offset-2 col-md-8 text-center bigger-text">
                    <p>templatemo provides a lot of free website templates for everyone. Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores, itaque vero facere laudantium voluptas temporibus mollitia nisi deleniti illo minima quisquam corporis possimus nesciunt.</p>
                </div>
